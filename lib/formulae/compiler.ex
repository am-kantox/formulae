defmodule Formulae.Compiler do
  ...
  case Application.compile_env(:formulae, :compiler) do
    ...
    _ ->
      ...
      def formulas(id, name), do: GenServer.call({:via, Registry, {id, name}}, :state).formulas
      ...
      def eval(_id, _name, %Formulae{} = formula, binding), do: Formulae.eval(formula, binding)
      ...
      def eval(id, name, {formula, options}, binding),
        do: GenServer.call({:via, Registry, {id, name}}, {:eval, {{formula, options}, binding}})
      ...
      @impl GenServer
      @doc false
      def handle_cast({:compile, {formula, _options}}, %__MODULE__{formulas: formulas} = state)
          when is_map_key(formulas, formula),
          do: {:noreply, state}
